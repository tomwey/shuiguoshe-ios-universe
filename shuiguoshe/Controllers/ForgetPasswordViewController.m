...
- (void)viewDidLoad {
    ...
    // 注册按钮
    _regBtn = createButton(@"btn_submit.png", self, @selector(doSubmit));
...
- (void)doSubmit
{
    ...
    __block ForgetPasswordViewController* me = self;
    [[UserService sharedService] forgetPassword:_currentUser completion:^(BOOL succeed, NSString* errorMsg) {
        if ( succeed ) {
